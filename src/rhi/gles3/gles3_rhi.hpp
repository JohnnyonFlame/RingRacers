...
struct Gles3FramebufferKey
{
	Handle<Texture> color;
	std::optional<Handle<Renderbuffer>> depth_stencil;

	bool operator==(const Gles3FramebufferKey& rhs) const noexcept
	{
		return color == rhs.color && depth_stencil == rhs.depth_stencil;
	}

	bool operator!=(const Gles3FramebufferKey& rhs) const noexcept { return !(*this == rhs); }
};

} // namespace srb2::rhi

// To make sure the compiler selects the struct specialization of std::hash for Gles3FramebufferKey,
// we need to split the namespace declarations _before_ the instantiation of std::unordered_map.

template <>
struct std::hash<srb2::rhi::Gles3FramebufferKey>
{
	std::size_t operator()(const srb2::rhi::Gles3FramebufferKey& key) const
	{
...
class Gl2Rhi final : public Rhi
{
	...
	std::unordered_map<Gles3FramebufferKey, uint32_t> framebuffers_ {16};
