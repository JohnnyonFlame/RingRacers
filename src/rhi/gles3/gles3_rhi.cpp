...
{
	gladLoadGLES3Loader((GLADloadproc)(load_func));
}
...
rhi::Handle<rhi::Texture> Gl2Rhi::create_texture(const rhi::TextureDesc& desc)
{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, map_texture_filter(desc.min));
	GL_ASSERT;
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, map_texture_filter(desc.mag));
	GL_ASSERT;
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, map_texture_wrap(desc.u_wrap));
	GL_ASSERT;
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, map_texture_wrap(desc.v_wrap));
	GL_ASSERT;
	glTexImage2D(GL_TEXTURE_2D, 0, internal_format, desc.width, desc.height, 0, format, GL_UNSIGNED_BYTE, nullptr);
	GL_ASSERT;
...
)
{
	...
	glBindTexture(GL_TEXTURE_2D, t.texture);
	GL_ASSERT;
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, map_texture_wrap(u_wrap));
	GL_ASSERT;
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, map_texture_wrap(v_wrap));
	GL_ASSERT;
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, map_texture_filter(min));
	GL_ASSERT;
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, map_texture_filter(mag));
	GL_ASSERT;
}
...
rhi::Handle<rhi::Pipeline> Gl2Rhi::create_pipeline(const PipelineDesc& desc)
{
	...
	glShaderSource(vertex, vert_sources.size(), vert_sources.data(), NULL);
	...
	glShaderSource(fragment, frag_sources.size(), frag_sources.data(), NULL);
...
void Gl2Rhi::begin_render_pass(Handle<GraphicsContext> ctx, const RenderPassBeginInfo& info)
{
	...
	auto fb_itr = framebuffers_.find(Gles3FramebufferKey {info.color_attachment, info.depth_stencil_attachment});
	if (fb_itr == framebuffers_.end())
	{
		...
		fb_itr = framebuffers_
					 .insert(
						 {Gles3FramebufferKey {info.color_attachment, info.depth_stencil_attachment},
...
void Gl2Rhi::bind_pipeline(Handle<GraphicsContext> ctx, Handle<Pipeline> pipeline)
{
	...
	if (desc.depth_stencil_state)
	{
		if (desc.depth_stencil_state->depth_test)
		{
			...
			GL_ASSERT;
			glDepthMask(desc.depth_stencil_state->depth_write ? GL_TRUE : GL_FALSE);
			GL_ASSERT;
		}
		else
		{
		...
		if (desc.depth_stencil_state->stencil_test)
		{
			...
			glStencilMaskSeparate(GL_FRONT, stencil_front_write_mask_);
			GL_ASSERT;
			glStencilMaskSeparate(GL_BACK, stencil_back_write_mask_);
			GL_ASSERT;
		}
		else
		{
...
void Gl2Rhi::bind_uniform_set(Handle<GraphicsContext> ctx, uint32_t slot, Handle<UniformSet> set)
{
	...
	for (size_t i = 0; i < us.uniforms.size(); i++)
	{
		...
		auto visitor = srb2::Overload {
			[&](const float& value)
			{
				glUniform1f(pipeline_uniform, value);
				GL_ASSERT;
			},
			[&](const glm::vec2& value)
			{
				glUniform2f(pipeline_uniform, value.x, value.y);
				GL_ASSERT;
			},
			[&](const glm::vec3& value)
			{
				glUniform3f(pipeline_uniform, value.x, value.y, value.z);
				GL_ASSERT;
			},
			[&](const glm::vec4& value)
			{
				glUniform4f(pipeline_uniform, value.x, value.y, value.z, value.w);
				GL_ASSERT;
			},
			[&](const int32_t& value)
			{
				glUniform1i(pipeline_uniform, value);
				GL_ASSERT;
			},
			[&](const glm::ivec2& value)
			{
				glUniform2i(pipeline_uniform, value.x, value.y);
				GL_ASSERT;
			},
			[&](const glm::ivec3& value)
			{
				glUniform3i(pipeline_uniform, value.x, value.y, value.z);
				GL_ASSERT;
			},
			[&](const glm::ivec4& value)
			{
				glUniform4i(pipeline_uniform, value.x, value.y, value.z, value.w);
				GL_ASSERT;
			},
			[&](const glm::mat2& value)
			{
				glUniformMatrix2fv(pipeline_uniform, 1, false, glm::value_ptr(value));
				GL_ASSERT;
			},
			[&](const glm::mat3& value)
			{
				glUniformMatrix3fv(pipeline_uniform, 1, false, glm::value_ptr(value));
				GL_ASSERT;
			},
			[&](const glm::mat4& value)
			{
				glUniformMatrix4fv(pipeline_uniform, 1, false, glm::value_ptr(value));
				GL_ASSERT;
			},
		};
...
void Gl2Rhi::bind_binding_set(Handle<GraphicsContext> ctx, Handle<BindingSet> set)
{
	...
	for (auto& texture_binding : bs.textures)
	{
		...
		GL_ASSERT;
		glBindTexture(GL_TEXTURE_2D, texture_gl_name);
		GL_ASSERT;
...
void Gl2Rhi::read_pixels(Handle<GraphicsContext> ctx, const Rect& rect, PixelFormat format, tcb::span<std::byte> out)
{
	...
	GLenum read_buffer = is_back ? GL_BACK : GL_COLOR_ATTACHMENT0;
	glReadBuffer(read_buffer);
	GL_ASSERT;

	glReadPixels(rect.x, rect.y, rect.w, rect.h, layout, type, out.data());
	GL_ASSERT;
}
...
void Gl2Rhi::finish()
{
	...
	{
		glDeleteFramebuffers(1, (GLuint*)&fbset.second);
		GL_ASSERT;
	}
	framebuffers_.clear();
}
...
)
{
	...
	GL_ASSERT;
	glCopyTexSubImage2D(GL_TEXTURE_2D, 0, dst_region.x, dst_region.y, src_region.x, src_region.y, dst_region.w, dst_region.h);
	GL_ASSERT;
}
